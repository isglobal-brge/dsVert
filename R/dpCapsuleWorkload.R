...
.dsvert_dp_capsule_binary_glmm_quadrature_v1 <- function() {
  # Nine fixed Gauss--Hermite nodes integrate the standard-normal random
  # intercept deterministically; neither the nodes nor their weights are
  # analyst-controlled protocol inputs.
  list(
    nodes = c(-3.19099320178153, -2.26658058453184, -1.46855328921667,
              -0.723551018752838, 0, 0.723551018752838,
              1.46855328921667, 2.26658058453184, 3.19099320178153),
    weights = c(0.0000396069772632644, 0.00494362427553695,
                0.0884745273943766, 0.432651559002556,
                0.720235215606051, 0.432651559002556,
                0.0884745273943766, 0.00494362427553695,
...
.dsvert_dp_capsule_binary_random_slope_candidates <- function(
  ...
  candidates <- lapply(value, function(candidate) {
    ...
    if (!is.numeric(beta) || !is.null(names(beta)) || length(beta) != dimension ||
        anyNA(beta) || any(!is.finite(beta)) || any(abs(beta) > 8) ||
        !is.numeric(covariance) || !is.null(names(covariance)) ||
        length(covariance) != effect_count^2 || anyNA(covariance) ||
        any(!is.finite(covariance)) || any(abs(covariance) > 16)) return(NULL)
    covariance <- matrix(as.numeric(covariance), effect_count, effect_count,
                         byrow = TRUE)
    ...
    eta_bound <- sum(abs(beta)) + sqrt(2) * max(abs(quadrature$nodes)) *
      sum(abs(root))
    loss_bound <- log1p(exp(eta_bound))
    if (!is.finite(loss_bound) || loss_bound <= 0) return(NULL)
    list(beta = as.numeric(beta), covariance = covariance, loss_bound = loss_bound)
  })
  if (any(vapply(candidates, is.null, logical(1L)))) return(list())
  keys <- vapply(candidates, function(candidate) .dsvert_dp_canonical_json(list(
...
.dsvert_dp_capsule_gaussian_spec <- function(
  ...
  if (raw$version %in% c(unname(.DSVERT_DP_GLM_GRID_CROSS_SPEC_VERSIONS),
    ...
    return(list(kind = if (identical(raw$version, "lmm_grid_cross_v1"))
  ...
  if (identical(version, "multinomial_grid_v1")) {
    ...
    if (!setequal(names(raw), expected)) {
      stop("Invalid multinomial likelihood-grid specification.",
           call. = FALSE)
    }
    outcome <- .dsvert_dp_capsule_column_reference(
      raw$outcome, "multinomial outcome")$reference
    predictors <- raw$predictors
    if (is.list(predictors) && is.null(names(predictors)) &&
        all(vapply(predictors, function(value) {
          is.character(value) && length(value) == 1L && !is.na(value)
    ...
    predictors <- if (is.character(predictors) && length(predictors) &&
      ...
      sort(vapply(predictors, function(value) {
        .dsvert_dp_capsule_column_reference(
          value, "multinomial fixed predictor")$reference
      }, character(1L)), method = "radix")
    } else {
      character()
    }
    ...
    levels <- tryCatch(
      .dsvert_canonical_label_values(
        levels, "multinomial outcome levels", allow_na = FALSE,
        allow_blank = FALSE),
      error = function(error) character())
    reference <- raw$reference
    if (!is.character(reference) || length(reference) != 1L ||
        is.na(reference) || !nzchar(reference)) reference <- NA_character_
    ...
    if (length(levels) < 3L || anyDuplicated(levels) ||
        !identical(levels, sort(levels, method = "radix")) ||
        !reference %in% levels || !isTRUE(raw$intercept) ||
        !length(predictors) || outcome %in% predictors ||
        !isTRUE(beta_valid) || length(beta_grid) > 256L) {
      stop("Invalid multinomial likelihood-grid model terms.",
           call. = FALSE)
    }
    if (!is.logical(require_public_bounds) ||
        length(require_public_bounds) != 1L || is.na(require_public_bounds)) {
    ...
    if (isTRUE(require_public_bounds) &&
        (!dataset %in% names(policy$datasets) ||
         !outcome %in% names(policy$categorical_levels) ||
         any(!predictors %in% names(policy$numeric_bounds)))) {
      stop("The multinomial grid specification lacks public bounds.",
           call. = FALSE)
    }
    return(list(
      version = version, dataset = dataset, outcome = outcome,
  ...
  if (identical(version, "binary_random_slope_grid_v1")) {
    expected <- c(
      "version", "dataset", "outcome", "cluster", "predictors",
      "random_slopes", "intercept", "max_patients_per_cluster",
      "candidate_grid")
    if (!setequal(names(raw), expected)) {
      stop("Invalid binary random-slope GLMM specification.", call. = FALSE)
    }
    outcome <- .dsvert_dp_capsule_column_reference(
      raw$outcome, "GLMM outcome")$reference
    cluster <- .dsvert_dp_capsule_column_reference(
      raw$cluster, "GLMM cluster")$reference
    normalize_references <- function(value, what) {
      if (is.list(value) && is.null(names(value))) value <- unlist(value, use.names = FALSE)
      if (!is.character(value) || !length(value) || !is.null(names(value)) ||
          anyNA(value) || anyDuplicated(value)) return(character())
    ...
    if (!length(candidates) || length(candidates) > 128L) {
      stop("Invalid binary random-slope GLMM candidate grid.", call. = FALSE)
    }
    if (!is.logical(require_public_bounds) ||
        length(require_public_bounds) != 1L || is.na(require_public_bounds)) {
      stop("Invalid Gaussian specification validation mode.", call. = FALSE)
    ...
    if (isTRUE(require_public_bounds) &&
        (!dataset %in% names(policy$datasets) ||
         any(!c(outcome, predictors) %in% names(policy$numeric_bounds)) ||
         !cluster %in% names(policy$categorical_levels))) {
      stop("The binary random-slope GLMM specification lacks public bounds.",
           call. = FALSE)
    }
    return(list(
      version = version, dataset = dataset, outcome = outcome, cluster = cluster,
      ...
      candidate_grid = lapply(candidates, function(candidate) list(
        ...
        covariance = unname(as.vector(t(candidate$covariance))))),
      kind = "binary_random_slope_grid"))
  }
  if (identical(version, "poisson_random_intercept_grid_v1")) {
    expected <- c(
      "version", "dataset", "outcome", "cluster", "predictors",
      "intercept", "max_patients_per_cluster", "max_outcome", "beta_grid",
      "variance_grid")
    if (!setequal(names(raw), expected)) {
      stop("Invalid Poisson random-intercept GLMM specification.",
           call. = FALSE)
    }
    outcome <- .dsvert_dp_capsule_column_reference(
    ...
    variance_grid <- raw$variance_grid
    if (is.list(variance_grid) && is.null(names(variance_grid))) {
      variance_grid <- unlist(variance_grid, use.names = FALSE)
    }
    beta_grid <- raw$beta_grid
    if (!is.list(beta_grid) || !length(beta_grid) || !is.null(names(beta_grid))) {
      beta_grid <- list()
    ...
    beta_valid <- length(beta_grid) && all(vapply(beta_grid, function(beta) {
      is.numeric(beta) && length(beta) == expected_dimension &&
        all(is.finite(beta)) && all(abs(beta) <= 8) && sum(abs(beta)) <= 16
    }, logical(1L)))
    beta_keys <- if (isTRUE(beta_valid)) vapply(beta_grid, function(beta) {
      .dsvert_dp_canonical_json(as.list(beta))
    }, character(1L)) else character()
    ...
    if (!is.numeric(maximum) || length(maximum) != 1L || is.na(maximum) ||
        ...
        !length(predictors) || outcome %in% predictors ||
        cluster %in% c(outcome, predictors) || !is.numeric(variance_grid) ||
        !length(variance_grid) || anyNA(variance_grid) ||
        any(!is.finite(variance_grid)) || any(variance_grid < 0) ||
        any(variance_grid > 16) || any(diff(variance_grid) <= 0) ||
        !isTRUE(beta_valid) ||
        length(beta_grid) * length(variance_grid) > 256L) {
      stop("Invalid Poisson random-intercept GLMM model terms.",
           call. = FALSE)
    }
    if (!is.logical(require_public_bounds) ||
        length(require_public_bounds) != 1L || is.na(require_public_bounds)) {
    ...
    if (isTRUE(require_public_bounds) &&
        (!dataset %in% names(policy$datasets) ||
         any(!c(outcome, predictors) %in% names(policy$numeric_bounds)) ||
         !cluster %in% names(policy$categorical_levels))) {
      stop("The Poisson random-intercept GLMM specification lacks public bounds.",
           call. = FALSE)
    }
    return(list(
      version = version, dataset = dataset, outcome = outcome,
      ...
      max_patients_per_cluster = as.integer(maximum),
      max_outcome = as.integer(max_outcome), beta_grid = unname(beta_grid),
  ...
  if (identical(version, "binary_random_intercept_grid_v1")) {
    expected <- c(
      ...
      "variance_grid")
    if (!setequal(names(raw), expected)) {
      stop("Invalid binary random-intercept GLMM specification.",
           call. = FALSE)
    }
    outcome <- .dsvert_dp_capsule_column_reference(
      raw$outcome, "GLMM outcome")$reference
    ...
    predictors <- if (is.character(predictors) && length(predictors) &&
      ...
      sort(vapply(predictors, function(value) {
        .dsvert_dp_capsule_column_reference(
          value, "GLMM fixed predictor")$reference
      }, character(1L)), method = "radix")
    } else {
      character()
    }
    ...
    if (!is.numeric(maximum) || length(maximum) != 1L || is.na(maximum) ||
        ...
        !isTRUE(beta_valid) ||
        length(beta_grid) * length(variance_grid) > 256L) {
      stop("Invalid binary random-intercept GLMM model terms.",
           call. = FALSE)
    }
    if (!is.logical(require_public_bounds) ||
        length(require_public_bounds) != 1L || is.na(require_public_bounds)) {
    ...
    if (isTRUE(require_public_bounds) &&
        (!dataset %in% names(policy$datasets) ||
         any(!c(outcome, predictors) %in% names(policy$numeric_bounds)) ||
         !cluster %in% names(policy$categorical_levels))) {
      stop("The binary random-intercept GLMM specification lacks public bounds.",
           call. = FALSE)
    }
    return(list(
      version = version, dataset = dataset, outcome = outcome,
      cluster = cluster, predictors = unname(predictors), intercept = TRUE,
      max_patients_per_cluster = as.integer(maximum),
      beta_grid = unname(beta_grid), variance_grid = as.numeric(variance_grid),
      kind = "binary_random_intercept_grid"))
  }
  if (version %in% c("random_intercept_fixed_v2",
    ...
    predictors <- if (is.character(predictors) && length(predictors) &&
      ...
      sort(vapply(predictors, function(value) {
        .dsvert_dp_capsule_column_reference(
          value, "LMM fixed predictor")$reference
      }, character(1L)), method = "radix")
    } else {
      character()
    }
    ...
    if (is.list(grid) && is.null(names(grid))) {
      grid <- unlist(grid, use.names = FALSE)
    }
    if (!is.numeric(maximum) || length(maximum) != 1L || is.na(maximum) ||
        !is.finite(maximum) || maximum != floor(maximum) || maximum < 2 ||
        maximum > policy$unit_capacity || !isTRUE(raw$intercept) ||
    ...
    if (isTRUE(require_public_bounds) &&
        (!dataset %in% names(policy$datasets) ||
         any(!c(outcome, predictors) %in% names(policy$numeric_bounds)) ||
         !cluster %in% names(policy$categorical_levels))) {
      stop("The fixed-effect random-intercept LMM specification lacks public bounds.",
           call. = FALSE)
    }
    return(list(
      version = version, dataset = dataset, outcome = outcome,
  ...
  for (analysis_id in names(gaussian_specs)) {
    ...
    if (identical(spec$kind, "multinomial_grid")) {
      ...
      if (!identical(unname(kinds), c(
          ...
          length(owners) != 1L || length(unique(datasets)) != 1L ||
          !identical(datasets[[1L]], spec$dataset) ||
          !identical(outcome$levels, spec$levels)) {
        stop(paste(
          "A multinomial likelihood grid must be same-owner with a signed",
          "categorical outcome and numeric predictors."), call. = FALSE)
      }
      design_terms <- c("(Intercept)", spec$predictors)
      non_reference <- setdiff(spec$levels, spec$reference)
      design_dimension <- length(design_terms)
      candidate_count <- .dsvert_dp_capsule_coordinate_add(
        0L, length(spec$beta_grid))
      candidate_bounds <- lapply(spec$beta_grid, function(beta) {
        score_bound <- max(colSums(abs(matrix(
          beta, nrow = design_dimension, ncol = length(non_reference)))))
        list(beta = as.numeric(beta),
             loss_bound = 2 * score_bound + log(length(spec$levels)))
      })
      loss_bounds <- vapply(candidate_bounds, `[[`, numeric(1L), "loss_bound")
      raw_per_candidate <- ceiling(loss_bounds * grid_scale)
      statistic_maximum <- as.numeric(
      ...
      if (!is.finite(base_raw_l1) || !is.finite(base_raw_l2) ||
        ...
        stop(structure(
          list(message = paste(
            "The multinomial likelihood-grid public bounds exceed the",
            "certified exact-computation domain."), call = NULL,
            reason = "multinomial_grid_numeric_backend_unrepresentable"),
          class = c("dsvert_numeric_backend_unrepresentable", "error",
                    "condition")))
      }
      adjacency_multiplier <- .dsvert_dp_adjacency_multiplier(global_policy)
      ...
      gaussian_artifacts[[analysis_id]] <- list(
        ...
        candidate_order = "canonical_beta_grid_softmax_v1",
        candidate_loss_bounds = as.list(unname(loss_bounds)),
        numeric_grid_bits = grid_bits, coordinate_count = candidate_count,
        coordinate_order = paste(
    ...
    if (identical(spec$kind, "binary_random_slope_grid")) {
      variables <- c(spec$outcome, spec$predictors, spec$cluster)
      model_columns <- columns[variables]
      owners <- unique(vapply(model_columns, `[[`, character(1L), "owner_peer"))
      datasets <- vapply(model_columns, `[[`, character(1L), "dataset")
      ...
      if (!identical(unname(kinds), expected_kinds) || length(owners) != 1L ||
          ...
          !identical(datasets[[1L]], spec$dataset) ||
          !identical(c(outcome$lower, outcome$upper), c(0, 1))) {
        stop(paste("A binary random-slope GLMM specification must be same-owner",
                   "with a [0,1] numeric outcome and predictors plus a",
                   "categorical cluster."), call. = FALSE)
      }
      design_terms <- c("(Intercept)", spec$predictors)
      random_effect_order <- c("(Intercept)", spec$random_slopes)
      candidates <- .dsvert_dp_capsule_binary_random_slope_candidates(
        spec$candidate_grid, length(design_terms), random_effect_order)
      if (!length(candidates) || length(candidates) > 128L) {
        stop("The binary random-slope GLMM candidate grid is invalid.",
             call. = FALSE)
      }
      loss_bounds <- vapply(candidates, `[[`, numeric(1L), "loss_bound")
      raw_per_candidate <- ceiling(loss_bounds * grid_scale)
      ...
      if (!is.finite(base_raw_l1) || !is.finite(base_raw_l2) ||
          ...
          any(statistic_maximum > .dsvert_dp_exact_integer_limit)) {
        stop(structure(list(
          message = paste("The binary random-slope GLMM public bounds exceed",
                          "the certified exact-computation domain."), call = NULL,
          reason = "binary_random_slope_glmm_numeric_backend_unrepresentable"),
          class = c("dsvert_numeric_backend_unrepresentable", "error",
                    "condition")))
      }
      adjacency_multiplier <- .dsvert_dp_adjacency_multiplier(global_policy)
      ...
      names(predictor_bounds) <- spec$predictors
      gaussian_artifacts[[analysis_id]] <- list(
        version = "bounded-binary-random-slope-likelihood-grid-v1",
        spec_version = spec$version, analysis_id = analysis_id,
        dataset = spec$dataset, owner_peer = owners[[1L]],
        outcome = list(column = outcome$column, lower = outcome$lower,
                       upper = outcome$upper),
        ...
        numeric_grid_bits = grid_bits,
        coordinate_count = as.integer(length(candidates)),
        coordinate_order = "signed_candidate_grid_cluster_marginal_binary_negative_log_likelihood_v1",
        source_coordinate_scaling =
          "all_coordinates_already_on_common_numeric_lattice_v1",
        repeated_record_policy = paste(
    ...
    if (identical(spec$kind, "binary_random_intercept_grid")) {
      ...
      kinds <- vapply(model_columns, `[[`, character(1L), "kind")
      expected_kinds <- c(
        rep("numeric", 1L + length(spec$predictors)), "categorical")
      outcome <- columns[[spec$outcome]]
      if (!identical(unname(kinds), expected_kinds) || length(owners) != 1L ||
          length(unique(datasets)) != 1L ||
          !identical(datasets[[1L]], spec$dataset) ||
          !identical(c(outcome$lower, outcome$upper), c(0, 1))) {
        stop(paste(
          "A binary random-intercept GLMM specification must be same-owner",
          "with a [0,1] numeric outcome, numeric predictors and a",
          "categorical cluster."), call. = FALSE)
      }
      design_terms <- c("(Intercept)", spec$predictors)
      beta_count <- length(spec$beta_grid)
      variance_count <- length(spec$variance_grid)
      candidate_count <- .dsvert_dp_capsule_coordinate_add(
        0L, beta_count * variance_count)
      quadrature <- .dsvert_dp_capsule_binary_glmm_quadrature_v1()
      max_node <- max(abs(quadrature$nodes))
      candidates <- unlist(lapply(spec$variance_grid, function(variance) {
        lapply(spec$beta_grid, function(beta) {
          eta_bound <- sum(abs(beta)) + sqrt(2 * variance) * max_node
          list(beta = as.numeric(beta), variance = as.numeric(variance),
               loss_bound = log1p(exp(eta_bound)))
        })
      }), recursive = FALSE)
      loss_bounds <- vapply(candidates, `[[`, numeric(1L), "loss_bound")
      ...
      base_raw_l2 <- .dsvert_dp_capsule_l2_sqrt(
        sum(raw_per_candidate^2))
      if (!is.finite(base_raw_l1) ||
          !is.finite(base_raw_l2) ||
          any(!is.finite(statistic_maximum)) ||
          any(statistic_maximum > .dsvert_dp_exact_integer_limit)) {
        stop(structure(
          list(
            message = paste(
              "The binary random-intercept GLMM public bounds exceed the",
              "certified exact-computation domain."),
            call = NULL,
            reason = "binary_random_intercept_glmm_numeric_backend_unrepresentable"),
          class = c("dsvert_numeric_backend_unrepresentable", "error",
                    "condition")))
      }
      adjacency_multiplier <- .dsvert_dp_adjacency_multiplier(global_policy)
      ...
      gaussian_artifacts[[analysis_id]] <- list(
        ...
        candidate_loss_bounds = as.list(unname(loss_bounds)),
        numeric_grid_bits = grid_bits,
        coordinate_count = candidate_count,
        coordinate_order = paste(
    ...
    if (identical(spec$kind, "poisson_random_intercept_grid")) {
      variables <- c(spec$outcome, spec$predictors, spec$cluster)
      model_columns <- columns[variables]
      owners <- unique(vapply(
        model_columns, `[[`, character(1L), "owner_peer"))
      ...
      if (!identical(unname(kinds), expected_kinds) || length(owners) != 1L ||
          ...
          !identical(c(outcome$lower, outcome$upper),
                     c(0, as.numeric(spec$max_outcome)))) {
        stop(paste(
          "A Poisson random-intercept GLMM specification must be same-owner",
          "with a [0,max_outcome] integer numeric outcome, numeric predictors",
          "and a categorical cluster."), call. = FALSE)
      }
      design_terms <- c("(Intercept)", spec$predictors)
      beta_count <- length(spec$beta_grid)
      variance_count <- length(spec$variance_grid)
      ...
      candidates <- unlist(lapply(spec$variance_grid, function(variance) {
        lapply(spec$beta_grid, function(beta) {
          eta_bound <- sum(abs(beta)) + sqrt(2 * variance) * max_node
          loss_bound <- exp(eta_bound) + spec$max_outcome * eta_bound +
            lgamma(spec$max_outcome + 1)
          list(beta = as.numeric(beta), variance = as.numeric(variance),
               loss_bound = loss_bound)
        })
      ...
      if (!is.finite(base_raw_l1) ||
        ...
        stop(structure(
          list(
            message = paste(
              "The Poisson random-intercept GLMM public bounds exceed the",
              "certified exact-computation domain."),
            call = NULL,
            reason = "poisson_random_intercept_glmm_numeric_backend_unrepresentable"),
          class = c("dsvert_numeric_backend_unrepresentable", "error",
                    "condition")))
      }
      adjacency_multiplier <- .dsvert_dp_adjacency_multiplier(global_policy)
      ...
      names(predictor_bounds) <- spec$predictors
      gaussian_artifacts[[analysis_id]] <- list(
        version = "bounded-poisson-random-intercept-likelihood-grid-v1",
        spec_version = spec$version, analysis_id = analysis_id,
        dataset = spec$dataset, owner_peer = owners[[1L]],
        outcome = list(column = outcome$column, lower = outcome$lower,
                       upper = outcome$upper),
    ...
    if (identical(spec$kind, "random_intercept_fixed")) {
      variables <- c(spec$outcome, spec$predictors, spec$cluster)
      model_columns <- columns[variables]
      owners <- unique(vapply(
        model_columns, `[[`, character(1L), "owner_peer"))
      ...
      if (!identical(unname(kinds), expected_kinds) || length(owners) != 1L ||
          length(unique(datasets)) != 1L ||
          !identical(datasets[[1L]], spec$dataset)) {
        stop("A fixed-effect random-intercept LMM specification must be same-owner with numeric outcome and predictors plus a categorical cluster.",
             call. = FALSE)
      }
      cluster_capacity <- as.integer(spec$max_patients_per_cluster)
      design_terms <- c("(Intercept)", spec$predictors)
      design_count <- length(design_terms)
      gram_count <- as.double(design_count) * (design_count + 1) / 2
      summary_count <- .dsvert_dp_capsule_coordinate_add(
        0L, gram_count + design_count + 1L)
      model_coordinate_count <- .dsvert_dp_capsule_coordinate_add(
        0L, (cluster_capacity + 1) * (summary_count + 1L))
      statistic_product_maximum <-
        as.double(capacity) * cluster_capacity * grid_scale
      raw_l1_base <- (3 + summary_count *
        (1 + 2 * cluster_capacity^2)) * grid_scale
      raw_l2_base <- .dsvert_dp_capsule_l2_sqrt(
        (3 + summary_count * (1 + 4 * cluster_capacity^4)) *
          grid_scale^2)
      required_signed_bits <- as.integer(ceiling(log2(capacity + 1))) +
        as.integer(ceiling(log2(cluster_capacity + 1))) +
        grid_bits + 3L
      if (!is.finite(statistic_product_maximum) ||
          statistic_product_maximum > .dsvert_dp_exact_integer_limit ||
          !is.finite(raw_l1_base) ||
          raw_l1_base > .dsvert_dp_exact_integer_limit ||
          !is.finite(required_signed_bits) || required_signed_bits >= 127L) {
        stop(structure(
          list(
            message = paste(
              "The fixed-effect random-intercept LMM public bounds exceed",
              "the certified exact-computation domain."),
            call = NULL,
            reason = "random_intercept_fixed_numeric_backend_unrepresentable",
            required_signed_bits = required_signed_bits),
          class = c("dsvert_numeric_backend_unrepresentable", "error",
                    "condition")))
      }
      adjacency_multiplier <- .dsvert_dp_adjacency_multiplier(global_policy)
      raw_l1 <- adjacency_multiplier * raw_l1_base
      raw_l2 <- adjacency_multiplier * raw_l2_base
      outcome <- columns[[spec$outcome]]
      cluster <- columns[[spec$cluster]]
      predictor_bounds <- lapply(spec$predictors, function(variable) {
        column <- columns[[variable]]
      ...
      gaussian_artifacts[[analysis_id]] <- list(
        ...
        statistic_maximum = c(
          capacity, rep(capacity * grid_scale, summary_count),
          rep(group_maximum, cluster_capacity)),
        source_raw_l1_sensitivity = raw_l1,
        source_raw_l2_sensitivity = raw_l2,
        natural_l1_sensitivity = raw_l1 / grid_scale,
        natural_l2_sensitivity = raw_l2 / grid_scale,
