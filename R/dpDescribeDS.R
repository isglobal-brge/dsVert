...
#'
#' Releases one sticky, purpose-bound vector containing quantized sufficient
#' statistics and fixed-grid histograms for a custodian-owned variable set.
#' No observed extrema, adaptive bins, exact counts, or fractional parts are
#' released.
#'
#' @param data_name Name of a registered protected data frame.
#' @param analysis_id Name of a custodian-owned describe specification.
#' @return A noisy integer vector and explicit sequential-composition metadata.
#' @keywords internal
dsvertDPDescribeDS <- function(data_name, analysis_id) {
  policy <- .dsvert_dp_policy()
  .dsvert_dp_require_global_allocator_bridge(policy)
  spec <- .dsvert_dp_describe_spec(policy, data_name, analysis_id)
  # All dimensional, grid, allocation, and sensitivity checks are public
  # policy checks and complete before the protected object is resolved.
  ...
  arguments <- list(
    ...
    grid_lengths = spec$grid_lengths,
    ...
    l2_sensitivity_bound = spec$l2_sensitivity_bound,
  ...
  query_hash <- .dsvert_dp_query_hash(
    secret, policy, dataset$public, "fixed_grid_describe", arguments)
  release <- .dsvert_dp_release(
    ...
    spec$sum_family_l1_sensitivity_bound,
    function(epsilon, delta, seed, mechanism_plan) {
      # Memoized releases return before this callback, so schema validation,
      # admission, grouping, quantization, and histograms run only once.
      exact <- .dsvert_dp_describe_statistics(data, policy, spec)
      laplace_plan <- .dsvert_dp_describe_coordinate_plan(spec, epsilon)
      sampled <- .dsvert_dp_sample_selected_int64(
        exact, laplace_plan$epsilons, laplace_plan$sensitivities,
        epsilon, delta, spec$l2_sensitivity_bound, seed,
        mechanism_plan)
      ...
      list(
        ...
        statistics = unname(noisy),
        coordinate_count = spec$coordinate_count,
        coordinate_layout = paste(
          "per_variable[count,qsum,qsumsq,",
          "histogram[grid_bins+invalid]]"),
        numeric_grid_bits = spec$numeric_grid_bits,
        numeric_grid_scale = spec$numeric_grid_scale,
        quantization =
          "round(z*scale) and round(z^2*scale) independently",
        max_abs_normalized_quantization_per_unit =
          0.5 / spec$numeric_grid_scale,
        allocation_names = spec$allocation_names,
        allocation_weights = spec$allocation_weights,
        epsilon_per_variable_family = laplace_plan$family_epsilon,
        epsilon_allocation_sum =
          spec$variable_count * sum(laplace_plan$family_epsilon),
        submechanism_count = if (identical(
          mechanism_plan$winner, "gaussian")) 1L else
          4L * spec$variable_count,
        ...
        family_sensitivity = spec$family_sensitivity,
        sum_family_l1_sensitivity_bound =
          spec$sum_family_l1_sensitivity_bound,
        l2_sensitivity_bound = spec$l2_sensitivity_bound,
        ...
        accuracy_95_abs_by_variable_family =
          .dsvert_dp_describe_accuracy(sampled$accuracy_95_abs, spec),
        accuracy_simultaneous_95_abs_by_variable_family =
          .dsvert_dp_describe_accuracy(
            sampled$accuracy_simultaneous_95_abs, spec),
    ...
    mechanism_plan = function(epsilon, delta) {
      ...
      .dsvert_dp_noise_plan(
        spec$coordinate_count, laplace_plan$epsilons,
        laplace_plan$sensitivities, epsilon, delta,
        spec$l2_sensitivity_bound, objective = "simultaneous_95_abs")
    },
    noise_context = list(
      dataset = dataset$public,
      method = "fixed_grid_describe",
      arguments = arguments))
