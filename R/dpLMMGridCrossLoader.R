...
# admit a grouped workload or register a remote source/release endpoint.
.dsvert_dp_lmm_cross_transport_layout <- function(manifest, artifact) {
  ...
  projection <- .dsvert_dp_grouped_cross_source_blocks(artifact, start)
  # Same transport-layout hash domain as the existing source protocol. Grouped
  # registration remains closed; this explicit single-LMM projection is local.
...
.dsvert_dp_lmm_cross_source_context <- function(policy, manifest,
  ...
  if (!identical(analysis_id, spec$analysis_id) ||
      !identical(.dsvert_dp_capsule_source_manifest_capsule_id(source_contract),
                 manifest$capsule_identity$capsule_id)) .dsvert_dp_grouped_cross_fail()
...
.dsvert_dp_lmm_cross_load_handoff <- function(policy, secret, manifest,
  ...
  alignment <- .dsvert_dp_lmm_cross_source_alignment(spec, policy, ss,
  ...
  variables <- c(unlist(spec$predictor_order, use.names = FALSE),
    spec$outcome$reference, spec$grouping$reference)
  loaded <- .dsvert_dp_capsule_source_with_store(policy, secret, function(con) {
    .dsvert_dp_capsule_source_transaction(con, {
      ...
      if (is.null(state) || !isTRUE(state$complete) ||
          ...
          state$next_chunk_index != 0) .dsvert_dp_grouped_cross_fail()
      ...
      for (variable in variables) for (kind in c("value", "validity")) {
  ...
  .dsvert_dp_lmm_cross_source_handoff(contract, policy, schema_manifest,
    source_hash, loaded$values, loaded$validities, ss, source_contract$capsule_id)
}
