...
import json,pathlib,re,datetime,hashlib,runpy
root=pathlib.Path(__file__).resolve().parent
policy_root=root.parent/'dsvert-crossowner/dsVert/inst/cross-grid-v2/integrator-validation'
policy=json.loads((policy_root/'release-capacity.json').read_text())
rescore=runpy.run_path(str(policy_root/'rescore-release-capacity.py'))['rescore']
blob=(root/'RESULTS.jsonl').read_bytes()
originals=root/'artifacts/capacity-rescore-inputs';originals.mkdir(exist_ok=True)
(originals/(hashlib.sha256(blob).hexdigest()+'.jsonl')).write_bytes(blob)
rows=[rescore(json.loads(l),policy) for l in blob.splitlines() if l]
for r in rows:
 ...
 if r['status']=='FAIL':
  if 'fixed-work Gaussian table is outside the certified support' in t:r['failure_reason']='Discrete-Gaussian plan unavailable: fixed-work Gaussian table is outside certified support.'
   ...
   exceeded=[]
   if r.get('seconds') is not None and r['seconds']>policy['capacity_seconds']:exceeded.append(f"release duration {r['seconds']} seconds exceeds {policy['capacity_seconds']} seconds")
