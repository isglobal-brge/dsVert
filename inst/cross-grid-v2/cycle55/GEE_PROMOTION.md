...
**Decision: PENDING. No n2000 release or capacity promotion is asserted.**
...
| r7 n4 independence-rho0 real DP preflight | PASS: bitwise oracle, sticky/tamper/cold; 1,958.903 s, 1,331,068,356 RPC bytes | PASS: bitwise oracle, sticky/tamper/cold; 2,041.136 s, 1,419,353,306 RPC bytes |
...
## Urgent de-gate correction — 2026-09-20 13:43 UTC

Both r7 n4 preflights have passed. Poisson completed at 11:30:18 UTC,
exit 0, bitwise-oracle/sticky/tamper/cold markers all true: 2,041.136 seconds
through first publication and 1,419,353,306 serialized RPC bytes. Its source
manifest is unchanged and already includes 16-coordinate execution chunking;
no sampler patch or duplicate Poisson smoke was needed. The stale RUNNING
checkpoint did not reflect the completed result.

Paired suites are confirmatory and no longer gate release launch. Server
paired passed 22,597 assertions, zero failures/errors/skips/warnings; client
paired continues on pod16. Both remain required for the final promotion record.
Dispatcher 28004 was stopped before any release launch. The first fleet attempt
failed before release work because its root harness symlink was absent; raw
failure evidence is retained under `evidence/r7-fleet/failed-fleet-setup`.
The corrected fresh root is `/workspace/dsvert/gee-fixed-rho-r7-wave1`; its
harness link and source-bound oracle are checked before launch. Frozen package
source and sampler are unchanged.

Detached dispatcher **15768** launched binomial K3 on **pod11** (controller
874443, release driver 874445). Peer bootstrap has begun with 900/86400-second
leases. All other pods11–24 were busy at inspection; no competing work was
started. Five cells await distinct idle pods, one cell per pod, up to six
concurrent releases. No heavy job was started on pod4 and no LMM job was touched.
The live dispatch and collection states under `evidence/r7-fleet` supersede
older scheduling checkpoints. Both families remain **Promoted=No** pending
actual n2000, K2 recovery/tamper, capacity and paired evidence. Capacity scope
remains p<=3/C<=500, B4/J2, independence rho=0 with the unchanged 6h/256GB caps.

Validation: mocked orchestration checks passed K2/K3/K5 single-cell launches
without paired evidence and rejection of a failed smoke. Collector checks
passed complete-proof promotion and failed-release rejection. These are
controller tests, not additional mathematical or n2000 release evidence.
