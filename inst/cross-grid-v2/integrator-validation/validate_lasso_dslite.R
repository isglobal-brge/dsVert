#!/usr/bin/env Rscript
# LASSO integration harness reusing the Step-2 synthetic custodian lifecycle.
...
trace(".dsvert_dp_glm_grid_cross_transcript_stop", where = asNamespace("dsVertClient"),
  print = FALSE, tracer = quote({
    if (!inherits(error, "dsvert_dp_public_failure")) {
      cat("LASSO_CLIENT_FAILURE_CLASS", paste(class(error), collapse = "/"), "\n")
      cat("LASSO_CLIENT_FAILURE", conditionMessage(error), "\n")
      cat("LASSO_CLIENT_FUNCTIONS", paste(vapply(sys.calls(), function(x)
        if (is.symbol(x[[1]])) as.character(x[[1]]) else "<call>", character(1)),
        collapse = ";"), "\n")
    }
  }))
...
run <- function() {
  on.exit({
    for (peer in peers) tryCatch(print(peer$worker$run(function() {
      get0(".grid_synthetic_transport_events", .GlobalEnv)
    })), error = function(error) cat("TRANSPORT_DIAGNOSTIC_UNAVAILABLE\n"))
    we_close_peers(peers)
    if (identical(Sys.getenv("DSVERT_GRID_VALIDATION_KEEP_STATE"), "1")) {
      cat("SYNTHETIC_VALIDATION_STATE", state, "\n")
    } else unlink(state, recursive = TRUE)
  }, add = TRUE)
  ...
  for (peer in peers) peer$worker$run(function(server_dir) {
    assign(".grid_synthetic_transport_events", list(), .GlobalEnv)
    trace(".exact_gc_mark_failed", where = asNamespace("dsVert"), print = FALSE,
      tracer = quote({
        records <- get(".grid_synthetic_transport_events", .GlobalEnv)
        records[[length(records) + 1L]] <- list(event = "failed", code = code,
          operation_id = state$operation_id, status = state$status,
          time = as.character(Sys.time()),
          functions = vapply(sys.calls(), function(x)
            if (is.symbol(x[[1]])) as.character(x[[1]]) else "<call>", character(1)))
        assign(".grid_synthetic_transport_events", records, .GlobalEnv)
      }))
    trace(".exact_gc_abort_state", where = asNamespace("dsVert"), print = FALSE,
      tracer = quote({
        records <- get(".grid_synthetic_transport_events", .GlobalEnv)
        records[[length(records) + 1L]] <- list(event = "abort",
          operation_id = state$operation_id, status = state$status,
          failure_code = state$failure_code, time = as.character(Sys.time()),
          functions = vapply(sys.calls(), function(x)
            if (is.symbol(x[[1]])) as.character(x[[1]]) else "<call>", character(1)))
        assign(".grid_synthetic_transport_events", records, .GlobalEnv)
      }))
    ...
    assign(".grid_kernel_starts", 0L, .GlobalEnv)
    trace(".dsvert_dp_glm_grid_cross_start", where = asNamespace("dsVert"),
      print = FALSE, tracer = quote(assign(".grid_kernel_starts",
        get(".grid_kernel_starts", .GlobalEnv) + 1L, .GlobalEnv)))
  ...
  base <- cf(".dsvert_dp_lasso_cross_base")(contract, policy)
  candidates <- unlist(lapply(c(0.5, 0), function(lambda) lapply(beta,
    function(b) list(lambda = lambda, beta = b))), recursive = FALSE)
  lasso <- list(version = "dsvert-cross-owner-lasso-signed-contract-v1",
    spec = cf(".dsvert_dp_lasso_cross_spec")(list(version = "lasso_grid_cross_v1",
      analysis_id = "lasso", candidate_grid = candidates), base))
  lasso$signatures <- lapply(peers, function(peer) peer$worker$run(function(lasso) {
    identity <- dsVert:::.get_identity_keypair()
    dsVert:::.dsvert_relay_sign_message(dsVert:::.dsvert_dp_lasso_cross_message(lasso),
      identity$identity_sk)
  }, args = list(lasso)))
  cat("DSLITE_LASSO_CUSTODIAN_SIGNATURES_COMPLETE\n")
  ...
  invoke <- function() cf("dp_lasso_grid")(predictor_order, lasso, contract,
    policy, schema, datasources = conns)
  if (identical(Sys.getenv("DSVERT_GRID_VALIDATION_INTERRUPT"), "1")) {
    interrupted <- tryCatch(invoke(), error = identity)
    reached <- all(unlist(lapply(peers[owner_names[1:2]], function(peer)
      peer$worker$run(function() get(".grid_interrupted", .GlobalEnv)))))
    if (!reached) {
      for (peer in peers) print(peer$worker$run(function()
        get0(".grid_synthetic_failure", .GlobalEnv)))
    }
    stopifnot(inherits(interrupted, "error"), reached)
    cat("DSLITE_INTERRUPTED_AFTER_DURABLE_BATCHES\n")
  }
  kernel_starts <- function() unlist(lapply(peers[owner_names[1:2]], function(peer)
    peer$worker$run(function() get(".grid_kernel_starts", .GlobalEnv))))
  starts_before_recovery <- kernel_starts()
  ...
  if (identical(Sys.getenv("DSVERT_GRID_VALIDATION_REQUIRE_DURABLE_REUSE"), "1")) {
    stopifnot(identical(Sys.getenv("DSVERT_GRID_VALIDATION_INTERRUPT"), "1"),
      all(starts_before_recovery > 0), identical(kernel_starts(), starts_before_recovery))
    cat("DSLITE_RECOVERY_REUSED_ALL_AUTHENTICATED_BATCHES\n")
  }
  ...
  objectives <- vapply(lasso$spec$candidate_grid, function(candidate) {
    index <- which(vapply(contract$spec$beta_grid, function(b)
      identical(unlist(b), unlist(candidate$beta)), logical(1)))[[1]]
    penalty <- ceiling(n * 2^16 * candidate$lambda *
      sum(abs(unlist(candidate$beta)[-1])))
    (as.numeric(oracle$Released[-1][index]) + penalty)/(n * 2^16)
  }, numeric(1))
  lambdas <- vapply(lasso$spec$candidate_grid, `[[`, numeric(1), "lambda")
  selected <- vapply(unique(lambdas), function(lambda) {
    indices <- which(lambdas == lambda)
    indices[which.min(objectives[indices])]
  }, integer(1))
  stopifnot(identical(fit$selected_candidates, selected),
    identical(unname(fit$selected_objectives), unname(objectives[selected])))
  replay <- invoke()
  stopifnot(identical(replay$paths, fit$paths),
    identical(replay$provenance_certificate, fit$provenance_certificate),
    identical(replay$postprocessing_binding_sha256, fit$postprocessing_binding_sha256))
  tampered <- lasso
  tampered$spec$public_penalty_raw[[1]] <- tampered$spec$public_penalty_raw[[1]] + 1
  bad <- tryCatch(cf("dp_lasso_grid")(predictor_order, tampered, contract,
    policy, schema, datasources = conns), error = identity)
  stopifnot(inherits(bad, "dsvert_dp_public_failure"))
  cat("DSLITE_LASSO_ORACLE_BITWISE_EQUAL_STICKY_TAMPER_REJECTED\n")
  if (identical(Sys.getenv("DSVERT_GRID_VALIDATION_COLD"), "1")) {
