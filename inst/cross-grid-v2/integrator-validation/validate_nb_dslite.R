...
stopifnot(n > 0, identical(family, "nb"), epsilon %in% c(1, 4, 8),
...
run <- function() {
  ...
  maximum <- 4
  y <- pmin(maximum, rnbinom(n, size = 4, mu = exp(eta)))
  ...
  make_contract <- function(schema) {
    ...
    raw_spec <- list(version = paste0(family, "_grid_cross_v1"), analysis_id = "grid",
      ...
      beta_grid = beta, theta_grid = rep(4, length(beta)), max_outcome = maximum, alignment = alignment)
    spec <- cf(".dsvert_dp_nb_grid_cross_spec")(raw_spec, policy, authenticated)
    artifact <- cf(".dsvert_dp_nb_grid_cross_artifact")(spec)
    list(version = "dsvert-cross-owner-grid-signed-contract-v1", spec = spec,
      artifact = artifact, source_contract = cf(".dsvert_dp_nb_grid_cross_source_contract")(spec, artifact))
  }
  ...
  invoke <- function() cf("dp_nb_grid")(formula, data_name = "DA", analysis_id = "grid",
  ...
  elapsed <- system.time(fit <- tryCatch(cf("dp_nb_grid")(formula, data_name = "DA", analysis_id = "grid",
  ...
  plan <- list(Family=family,Rows=min(32,n),Predictors=predictors,Owners=owner_count,
    A=16,ThetaExponents=as.list(log2(unlist(spec$theta_grid))),GridBits=16,MaxOutcome=maximum,
  ...
  rejected <- tryCatch(cf("dp_nb_grid")(formula, "DA", "grid", tampered,
    policy, schema, datasources = conns), error = identity)
  stopifnot(inherits(rejected, "dsvert_dp_public_failure"))
  cat("DSLITE_NB_ORACLE_BITWISE_EQUAL_STICKY_TAMPER_REJECTED\n")
  if (identical(Sys.getenv("DSVERT_GRID_VALIDATION_COLD"), "1")) {
