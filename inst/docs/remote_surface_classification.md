...
| Unregistered endpoint names retained below guarded/test-only client code | 89 |
...
primitives. Nineteen retired endpoints have no executable client construction;
the remaining 86 retired endpoints plus three locally quarantined unregistered
...
| Class | Count | Meaning |
|---|---:|---|
| Production-safe / purpose-bound | 92 | Fixed status/schema, padded PSI, complete stateless Count, Frequency and Synopsis execution, typed-store, transport, capsule manifest/source, cross-owner checked-vector-multiplication, bounded registered-formal-GLM source ingress, selector-bound fresh formal-GLM and formal-Cox source relays, the closed provisioned formal-GLM host relay, and read-only completed formal-model certificate routes admitted by the default guard |
...
The executable tests assert this exact set, the exact 92-endpoint registration
