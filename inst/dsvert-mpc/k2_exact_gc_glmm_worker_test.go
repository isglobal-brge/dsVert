...
func groupedGLMMWorkerTestConfig(t *testing.T, role int, spec groupedGLMMSourceSpec, source *groupedGLMMSourceRecord, side *groupedRouteSidecar, directory, spool string, attempt int) exactGCWorkerConfig {
	t.Helper()
	key := crossGridStageTestKey(exactGCRole(role))
	session := sha256.Sum256([]byte(fmt.Sprintf("glmm worker transport/%s/%d", t.Name(), attempt)))
	master := sha256.Sum256([]byte(fmt.Sprintf("glmm worker transport master/%s/%d", t.Name(), attempt)))
	name := "garbler"
	if role == 1 {
	...
	return exactGCWorkerConfig{Version: exactGCWorkerConfigVersion, Role: name,
		...
		GarblerID: spec.Authorities[0], EvaluatorID: spec.Authorities[1], Purpose: groupedGLMMWorkerPurpose(spec),
		Operation: string(groupedGLMMWorkerOperation), RingBits: 128, VectorLen: len(spec.GLMM.Caps),
		SpoolDir: spool, MaxSpoolBytes: 1 << 30, TTLSeconds: 600,
		GroupedGLMM: &groupedGLMMWorkerInput{Spec: spec, Source: source, Routing: side,
			StoreDirectory: directory, StoreKey: base64.StdEncoding.EncodeToString(key[:])}}
}

func groupedGLMMWorkerTestSession(t *testing.T, config exactGCWorkerConfig) exactGCSession {
	t.Helper()
...
func TestGroupedGLMMWorkerOpaqueNativeIntegers(t *testing.T) {
	s, sources, side, _ := groupedGLMMSourceFixture(t)
	config := groupedGLMMWorkerTestConfig(t, 0, s, sources[0], side, filepath.Join(t.TempDir(), "durable"), exactGCTestSpool(t, "spool"), 0)
	// Deliberately not representable in a binary64 mantissa.
	config.GroupedGLMM.Spec.GLMM.Beta[0][0] = new(big.Int).Add(exactGCModulus(64), big.NewInt(1))
	raw, err := json.Marshal(config.GroupedGLMM)
	if err != nil {
		t.Fatal(err)
	}
	var encoded string
	...
	var recovered groupedGLMMWorkerInput
	if json.Unmarshal(raw, &recovered) != nil || recovered.Spec.GLMM.Beta[0][0].Cmp(config.GroupedGLMM.Spec.GLMM.Beta[0][0]) != 0 {
...
func TestGroupedGLMMWorkerSourceBindingRejectsReplacement(t *testing.T) {
	s, sources, side, _ := groupedGLMMSourceFixture(t)
	config := groupedGLMMWorkerTestConfig(t, 0, s, sources[0], side, filepath.Join(t.TempDir(), "durable"), exactGCTestSpool(t, "spool"), 0)
	session := groupedGLMMWorkerTestSession(t, config)
	prepared, err := groupedGLMMWorkerPrepare(config, session)
	if err != nil {
		t.Fatal(err)
	}
	...
	sources[0].MAC = groupedGLMMSourceMAC(prepared.key, *sources[0])
	changed, err := groupedGLMMWorkerPrepare(config, session)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := store.root.Remove("glmm-source.binding"); err != nil {
		t.Fatal(err)
	}
	if prepared.bindSource(store) == nil {
		t.Fatal("missing binding beside existing stage accepted")
	}
	for _, change := range []string{"purpose", "legacy-source", "lmm-source", "seed", "role", "spool-store"} {
		t.Run(change, func(t *testing.T) {
			bad := config
			input := *config.GroupedGLMM
			bad.GroupedGLMM = &input
			ss := session
			switch change {
			case "purpose":
				ss.Purpose += "/changed"
			...
			if _, err := groupedGLMMWorkerPrepare(bad, ss); err == nil {
...
func TestGroupedGLMMStagedSpoolWorkers(t *testing.T) {
	s, sources, side, rows := groupedGLMMSourceFixture(t)
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		for role := range paths {
			config := groupedGLMMWorkerTestConfig(t, role, s, sources[role], side, directories[role], spools[role], attempt)
		...
		if !exactGCTestBothDone(spools[0], spools[1]) {
			t.Fatal("GLMM staged spool workers timed out")
		}
		...
		for role := range results {
			...
			r := results[role]
			if r.Kind != "grouped-glmm-staged-ring128-share-v1" || len(r.StageReceipt) != 64 || len(r.StagePlanDigest) != 64 {
				t.Fatal("terminal staged evidence missing")
			}
			var err error
			opened[role], err = exactGCDecodeWorkerShares(r.Share, exactGCCircuitSpec{Operation: exactGCTruncateFloor, RingBits: 128, VectorLen: len(s.GLMM.Caps)})
			if err != nil {
				t.Fatal(err)
			}
			validity[role], err = unpackBoolsB64(r.ValidityShare, len(s.GLMM.Caps))
			if err != nil {
				t.Fatal(err)
			}
		...
		for j := range s.GLMM.Caps {
			want := groupedGLMMStagedOracle(s.GLMM, rows, j)
