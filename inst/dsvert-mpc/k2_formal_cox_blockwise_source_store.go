...
func (store *formalCoxBlockwiseSourceStore) acceptLocked(encoded, binding []byte) (
	bool, error) {
...
func (store *formalCoxBlockwiseSourceStore) Accept(encoded, binding []byte) (
	...
	return store.acceptLocked(encoded, binding)
}

// AcceptGuardedSamplerSourceOffers is the only source-store entrypoint for
// an update generated by the sticky sampler.  It validates both public offers
// against their signed one-draw provenance and their common guarded barrier
// before persisting this recipient's encrypted update.  It neither receives
// nor exposes a sampler share, seed, or transport secret.
func (store *formalCoxBlockwiseSourceStore) AcceptGuardedSamplerSourceOffers(
	dpPlan formalCoxDPPlan, trust jointDPBiomedicalGaussianWorkerTrustRoot,
	offers []formalCoxBlockwiseSamplerSourceOffer, binding []byte,
) (bool, error) {
	store.mu.Lock()
	defer store.mu.Unlock()
	if store.closed || store.owner == nil || store.session == nil ||
		store.session.context == nil || len(offers) != 2 {
		return false, fmt.Errorf("formal-cox: invalid sampler source store")
	}
	if err := formalCoxBlockwiseValidateGuardedSamplerSourceOfferBinding(
		store.session, dpPlan, trust, offers, binding); err != nil {
		return false, err
	}
	local := -1
	for index, peer := range store.plan.Policy.ComputePeers {
		if peer == store.recipient && offers[index].RecipientPeerName == peer {
			local = index
			break
		}
	}
	if local < 0 {
		return false, fmt.Errorf("formal-cox: sampler source offer lacks recipient")
	}
	return store.acceptLocked(offers[local].SourceEnvelope, binding)
}
