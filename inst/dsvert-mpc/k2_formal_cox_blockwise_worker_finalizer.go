...
const formalCoxBlockwiseWorkerFinalizerIngressDomain = "dsVert/formal-cox/blockwise-worker-finalizer-ingress/v1"
...
func formalCoxBlockwiseWorkerFinalizerIngressKey(secret []byte,
	binding formalFinalizerHandoffBinding,
	local formalFinalizerHandoffAuthority,
) ([32]byte, error) {
	var key [32]byte
	if len(secret) != sha256.Size || !formalCoxIsSHA256(binding.ArtifactID) ||
		...
		!formalCoxIsSHA256(binding.PlanSHA256) ||
		!formalCoxIsSHA256(binding.PinsetSHA256) ||
		!formalFinalizerHandoffAuthorityEqual(local, binding.Finalizer) {
		return key, fmt.Errorf("formal-cox: invalid worker finalizer ingress binding")
	}
	mac := hmac.New(sha256.New, secret)
	_, _ = mac.Write([]byte(formalCoxBlockwiseWorkerFinalizerIngressDomain + "|"))
	for _, value := range []string{
		binding.ArtifactID, binding.FinalPairRootSHA256, binding.PlanSHA256,
		binding.PinsetSHA256, local.PeerName, local.PeerID, local.Role,
	} {
...
// openFinalizerIngressAtRootV1 opens only the designated finalizer's durable
// ticket and ingress store.  Its encryption root is derived from that
// worker's recipient source secret; no relay or peer supplies a storage key.
func (bridge *formalCoxBlockwiseSourceBridge) openFinalizerIngressAtRootV1(
	opening *formalCoxBlockwiseOpeningStore,
	headers [2]formalCoxBlockwiseOpeningHandoffHeader,
	stateRoot string, production bool,
) (*formalFinalizerHandoffStore, error) {
	if bridge == nil || opening == nil || !filepath.IsAbs(stateRoot) ||
		filepath.Clean(stateRoot) != stateRoot ||
		(production && stateRoot != formalFinalizerHandoffStateRoot) {
		return nil, fmt.Errorf("formal-cox: invalid worker finalizer ingress root")
	}
	binding, err := formalCoxBlockwiseOpeningFinalizerBinding(opening, headers)
	if err != nil {
		return nil, err
	...
	if err != nil || local.PeerName != peer || local.PeerID != peerID ||
		!formalFinalizerHandoffAuthorityEqual(local, binding.Finalizer) ||
		formalFinalizerHandoffValidateBinding(binding, pins) != nil {
		return nil, fmt.Errorf("formal-cox: worker is not the finalizer")
	}
	storageRoot, err := formalCoxBlockwiseWorkerFinalizerIngressKey(
		secret, binding, local)
	if err != nil {
		return nil, err
	}
...
// IssueFinalizerTicketAtRootV1 fixes the finalizer transport key locally and
// returns only its signed public ticket.  The X25519 secret is never exposed
// to the worker caller or the relay.
func (bridge *formalCoxBlockwiseSourceBridge) IssueFinalizerTicketAtRootV1(
	opening *formalCoxBlockwiseOpeningStore,
	headers [2]formalCoxBlockwiseOpeningHandoffHeader,
	stateRoot string, production bool,
	...
	if bridge.closed || len(bridge.signingKey) != ed25519.PrivateKeySize {
		bridge.mu.Unlock()
		return zero, false, fmt.Errorf("formal-cox: worker finalizer bridge is unavailable")
	}
	privateKey := append(ed25519.PrivateKey(nil), bridge.signingKey...)
	bridge.mu.Unlock()
	defer clear(privateKey)
	ticket, secret, replayed, err := ingress.IssueTicketOnce(privateKey)
	clear(secret)
	if err != nil {
		return zero, false, err
	}
