...
func TestTrainingExplodeTrace(t *testing.T) {
    ring := NewRing63(20)
    ...
    for iter := 1; iter <= 10; iter++ {
        ...
        // Sigmoid: compare distributed vs SecureSigmoidLocal
        ...
        for i := 0; i < n; i++ {
            ...
            d := math.Abs(dv - pv)
            if d > maxSigDiff { maxSigDiff = d }
        }

        // Gradient with distributed sigmoid
        ...
        t.Logf("Iter %d: maxEta=%.4f maxSigDiff=%.2e gInt=%.6f beta=%v",
            iter, maxEta, maxSigDiff, gInt, beta)

        // Update with DISTRIBUTED gradient
        ...
        for j := range beta { beta[j] -= alpha * grad[j] }

        if maxSigDiff > 1.0 {
            t.Logf("  SIGMOID EXPLODED at iter %d", iter)
            break
        }
    }
}
