...
// DCFBatchEval evaluates the DCF on multiple inputs.
func DCFBatchEval(partyID int, key DCFKey, xs []uint64) []int64 {
	results := make([]int64, len(xs))
	for i, x := range xs {
		results[i] = DCFEval(partyID, key, x)
	}
	return results
}
...
// DPFGen generates DPF keys for the point function f(x) = beta if x == alpha, else 0.
func DPFGen(alpha uint64, beta int64, numBits int) (key0, key1 DCFKey) {
	// DPF is like DCF but WITHOUT the "add beta when Lose == Left" step.
	// The only nonzero output is at x == alpha.
	...
	for i := 0; i < numBits; i++ {
		...
		vCW := sign * (v1Lose - v0Lose - vAlpha)
		// DPF: NO beta addition here (unlike DCF)
		...
		cw := dcfCW{SeedCW: sCW, VCW: vCW, TCW_L: tCW_L, TCW_R: tCW_R}
		key0.CW[i] = cw
		key1.CW[i] = cw
	...
	// DPF final: add beta at the leaf
	key0.FinalCW = signFinal * (dcfConvertG(curS1) - dcfConvertG(curS0) - vAlpha + beta)
	key1.FinalCW = key0.FinalCW

	return
}

// DPFEval evaluates the DPF. Same as DCFEval (same key structure).
func DPFEval(partyID int, key DCFKey, x uint64) int64 {
	return DCFEval(partyID, key, x)
}
