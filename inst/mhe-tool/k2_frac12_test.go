...
func TestSigmoidFrac12FailRate(t *testing.T) {
    ring := NewRing63(12)
    
    // Test the problematic value with fracBits=12
    for _, x := range []float64{1.052, 1.5, 0.99, -0.5, 2.0} {
        ...
        t.Logf("fracBits=12: sigmoid(%.3f) failure rate: %d/%d = %.1f%%", x, fails, N, 100*float64(fails)/float64(N))
    }
}

func TestTrainingFrac12(t *testing.T) {
    ring := NewRing63(12)
    ...
    for iter := 1; iter <= 100; iter++ {
        ...
        if iter%25 == 0 {
            t.Logf("Iter %d: beta=%v", iter, beta)
        }
    }
    ...
    t.Logf("Distributed (fracBits=12): %v", beta)
