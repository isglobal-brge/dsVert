test_that("Poisson GLMM admission preserves integer outcomes and the complete variance grid", {
  for (owners in c(2L, 3L, 5L)) {
    f <- .lmm_handoff_fixture(owners, "poisson_glmm")
    ...
    expect_identical(.dsvert_dp_capsule_gaussian_spec(f$policy, "grouped",
      list(grouped = list(version = "poisson_glmm_grid_cross_v1", dataset = "cohort",
    ...
    expect_identical(handoff$plan$Numeric$family, "poisson")
    expect_identical(handoff$plan$Numeric$max_outcome, "4")
    ...
    expect_equal(outcome$maximum, 4)
...
test_that("Poisson GLMM durable terminal records retain private validity and reject LMM swaps", {
  f <- .lmm_handoff_fixture(family = "poisson_glmm")
