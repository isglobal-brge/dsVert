...
test_that("dynamic exact truncation and count guard complete over opaque spools", {
  ...
  alignment_terminal_call <- function(
    ...
    testthat::with_mocked_bindings(
      ...
      # The terminal fixture supplies a two-coordinate synthetic layout,
      # just as it supplies the parsed contract above. Projection validation
      # and the signed/encrypted terminal exchange remain real.
      .dsvert_dp_gaussian_cross_layout = function(manifest) {
        list(enabled = TRUE, private_start = 2,
             transport_coordinate_count = 2,
             blocks = list(list(start = 2, end = 2, length = 1)))
      },
      .package = "dsVert")
  }
