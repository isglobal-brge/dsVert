test_that("numeric imputation uses an intercept-only fallback", {
  D <- data.frame(x = c(1, NA, 3, NA, 5, 6))

  res <- dsvertImputeColumnDS("D", "x", "x_imp", seed = 11L)

  expect_equal(res$n_imputed, 2L)
  expect_equal(res$method, "bayesian_ridge_intercept")
...
test_that("binary imputation uses an intercept-only fallback", {
  D <- data.frame(x = factor(c("a", "b", NA, "b", NA, "a")))

  res <- dsvertImputeColumnDS("D", "x", "x_imp", seed = 33L)

  expect_equal(res$n_imputed, 2L)
  expect_equal(res$method, "bayesian_logit_intercept")
  expect_false(anyNA(D$x_imp))
